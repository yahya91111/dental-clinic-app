...
- `get_slot(week_start, day, period, room)` — read the TL's
  current slot.
- `find_swap_candidates(target_day, target_period)` — returns
  eligible doctors in the target period (filters out doctors
  on leave, permission, vacation, or with an active competing
  broadcast).
...
3. Call `find_swap_candidates(target_day, target_period)`:
...
5. Call `broadcast_swap_request(...)` with all candidate IDs.
...
AI: [calls get_slot(this_week, sunday, P2, ...)] — TL has Sunday P2 room 1
    [calls find_swap_candidates(sunday, P1)] — returns 3 candidates
...
AI: [calls broadcast_swap_request(...)]
...
AI: [calls find_swap_candidates(thursday, P3)] — empty

...
export type AnnounceOffer = {
  ...
  subjectName: string;
  /**
   * إبلاغُ **صدورِ الجدول** لا إبلاغُ غياب. الفرقُ في المستلِمين: إبلاغُ الغيابِ يستثني
   * القادةَ (وصلهم إشعارُهم التلقائيّ عن الطلب فلا يُكرَّر)، أمّا صدورُ الجدولِ فخبرٌ
   * لا إشعارَ تلقائيَّ له — فيصلُ **الجميعَ ومنهم القادةُ الآخرون**، ولا يُستثنى إلّا كاتبُه.
   * ونصُّه وحدَه هو ما يُرسَل: «نُشِر جدولُ الأسبوع…» بلا تفاصيلِ غياباتِ الأسبوع.
   */
  scope?: 'absence' | 'schedule';
};

...
// تلتصقُ بأوّلِ تبديلٍ في اليومِ دائمًا — فمَن تركتَه في شفتِ المساءِ تُعرَضُ شارتُه عندَ تبديلِ
// الظهرِ الذي مضى قبلَ ساعات، في ماضٍ لا شأنَ له به.
//
// والقطاعُ الجاري ينتهي عندَ **أوّلِ تبديلٍ لم ينقضِ بعد**، أي أوّلِ ما نهايتُه بعدَ الآن —
// وهذا يُصيبُ الحالتَين: أن نكونَ بينَ تبديلَين، وأن نكونَ داخلَ تبديلٍ نفسِه (فشفتُنا حينَها
// هو الذي انقضى للتوّ، وحائطُه هو هذا التبديلُ عينُه).
export const shiftWall = <T extends { start: number; end: number }>(seams: T[], nowMin: number): T | undefined =>
  seams.find((s) => s.end > nowMin);

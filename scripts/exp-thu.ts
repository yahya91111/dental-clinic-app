...
const rThu = distributeShiftWheel('thursday', 3, pool(docs.filter((d) => d.id !== 'd1')), wThu, true);
console.log(`الخميس — قبل: ${desc(canon['thursday']!)}`);
console.log(`الخميس — بعد: ${desc(rThu.slots)}  (ط1 غائب)`);
...
for (const d of docs) console.log(`  ${d.name}: ${soloCount.get(d.id) ?? 0}${d.id === 'd1' ? '  (انفرد الأحد، ثمّ غاب الخميس)' : ''}`);

// ═══════════════════════════════════════════════════════════════
// Test — Board scenario: separate_schedule (board OUTSIDE distribution)
// ═══════════════════════════════════════════════════════════════
// البورد له جدوله الخاص → لا يدخل توزيع العيادات/الاحتياطي إطلاقاً.
...
const WEEK_START = process.argv[2] || '2026-05-24';
...
async function main() {
  ...
  divider(`اختبار البورد separate_schedule · الأسبوع ${WEEK_START}`);
  console.log(`عدد العيادات: ${data.clinicCount}`);
  console.log(`البورد (${board.length}) خارج الجدول هذا الأسبوع: ${board.map((b) => b.name).join('، ') || 'لا يوجد'}`);
  const active = data.doctors.filter((d) => d.workStatus === 'active' && (d.groupTemplate.key === 'group_a' || d.groupTemplate.key === 'group_b'));
  const light = data.doctors.filter((d) => d.workStatus === 'light_duty');
  const vac = data.doctors.filter((d) => d.workStatus === 'vacation');
  console.log(`نشط (A/B): ${active.length} · تخفيف: ${light.length} · إجازة: ${vac.length}`);
  console.log('\nشفتات قروب A:');
  ...
  const result = await schedule.build({
    ...
    aShiftPlan: A_SHIFT,
    boardConfig: {
      scenario: { kind: 'separate_schedule' },

/* C1 — تطبيق القلب الجديد (كتابة) خلف العلم، عيادة الاختبار. نتحقّق من **أمان الكتابة**:
 * (أ) بعد التطبيق الجدول صالح (لا حجز مزدوج، كلّ شفت بدليقيتره، يوافق القلب الجديد).
 * (ب) لا تكرار (idempotent). (ج) مُبوّب بالعلم. (د) عيادةٌ أخرى → لا شيء. (هـ) قابلُ العودة. */
...
(async()=>{
  ...
  // (د) عيادةٌ خارج القائمة → لا شيء حتى مع المفتاح.
  newHeartConfig.mode='apply'; newHeartConfig.clinics=[CID];
  const other=await applyNewHeartRebalance({clinicId:'99999999-0000-0000-0000-000000000099',weekStart:W,label:'x'});
  check('(د) عيادةٌ خارج القائمة: لا تطبيق', other.applied===0);
  newHeartConfig.clinics=null;
  ...
  // (ج) المفتاح مطفأ → لا تطبيق.
  newHeartConfig.mode='off';
  const r3=await applyNewHeartRebalance({clinicId:CID,weekStart:W,label:'test'});
  check('(ج) المفتاح مطفأ: لا تطبيق', r3.applied===0);
  newHeartConfig.mode='off';

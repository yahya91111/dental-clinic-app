...
(async()=>{
  await build();
  // (د) عيادةٌ أخرى → لا شيء حتى مع العلم.
  ...
  check('(د) عيادةٌ أخرى: لا تطبيق', other.applied===0);
  ...
  // (ج) العلم مطفأ → لا تطبيق.
  process.env.NEW_HEART_APPLY='0';
  const r3=await applyNewHeartRebalance({clinicId:CID,weekStart:W,label:'test'});
  check('(ج) العلم مطفأ: لا تطبيق', r3.applied===0);
